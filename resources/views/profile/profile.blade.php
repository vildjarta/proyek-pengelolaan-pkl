...
<div class="row">
    {{-- LOGIKA TAMPILAN: --}}
    {{-- Hanya tampilkan input 'Kata Sandi Lama' jika user TIDAK punya google_id --}}
    
    @if(empty($user->google_id))
        {{-- USER MANUAL: Tampilkan Input Password Lama --}}
        <div class="col-md-12 mb-3">
            <label for="current_password">Kata Sandi Saat Ini</label>
            <input type="password" id="current_password" name="current_password" class="form-control" placeholder="Masukkan kata sandi lama">
            @error('current_password') 
                <div class="text-danger mt-1" style="font-size: 0.9rem;">{{ $message }}</div> 
            @enderror
        </div>
    @else
        {{-- USER GOOGLE: Beri pesan bahwa mereka bisa set password --}}
        <div class="col-md-12 mb-3">
            <div class="alert alert-success d-flex align-items-center" role="alert">
                <i class="fa fa-check-circle me-2"></i>
                <div>
                    <strong>Akun Google Terhubung.</strong> Silakan buat kata sandi di bawah ini agar Anda bisa login secara manual (tanpa Google) di masa depan.
                </div>
            </div>
        </div>
    @endif

    {{-- Input Password Baru (Selalu Muncul) --}}
