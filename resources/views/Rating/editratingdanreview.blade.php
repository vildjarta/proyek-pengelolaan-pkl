...
<head>
  <meta charset="UTF-8">
  <title>Edit Rating & Review | SIMPKL-TI</title>
...
<body>
  ...
  <div class="main-content-wrapper">
    <div class="full-container">
      <h1 class="page-title">Edit Rating & Review</h1>
      ...
      {{-- FORM EDIT --}}
      <form action="{{ route('ratingdanreview.update', $ratingdanreview) }}" method="POST" class="rating-form">
        @csrf
        @method('PUT')
          ...
          <div class="rating-stars">
            @for ($i = 5; $i >= 1; $i--)
              <input type="radio" id="star{{ $i }}" name="rating" value="{{ $i }}"
                     {{ (int) old('rating', $ratingdanreview->rating) === $i ? 'checked' : '' }} required>
        ...
        <div class="d-flex justify-content-end gap-3">
          <a href="{{ route('lihatratingdanreview', ['id_perusahaan' => $ratingdanreview->id_perusahaan]) }}" class="btn btn-secondary">
