    ...
    <div class="menu-list">
        @auth
            @php
                ...
                $showDataPerusahaan = in_array($userRole, [
                    'koordinator',
                    'staff',
                    'mahasiswa',
                    'ketua_prodi',
                ...
                $showNilaiMahasiswa = in_array($userRole, [
                    'koordinator',
                    'mahasiswa',
                    'dosen_penguji',
                    'dosen_pembimbing',
                ...
                $showRatingPerusahaan = in_array($userRole, [
                    'koordinator',
                    'mahasiswa',
                    'staff',
                    'ketua_prodi',
                ...
                $isBimbinganOpen =
                    request()->is('datadosenpembimbing*') ||
                    request()->is('dosen_penguji*') ||
                    request()->is('dosen') ||
                    request()->is('dosen/*');
                ...
                $isPenilaianOpen =
                    ...
                    request()->is('penilaian-penguji*') ||
                    request()->is('penilaian_perusahaan*') ||
                    request()->is('nilai*') ||
                    request()->is('ratingperusahaan*');
            ...
            <ul class="dropdown-menu {{ $currentRoute == 'home' ? '' : 'collapsed' }}"
                aria-hidden="{{ $currentRoute == 'home' ? 'false' : 'true' }}">
            ...
            @if ($showDataMahasiswa || $showTranskrip)
                ...
                <ul class="dropdown-menu {{ $isDataAkademikOpen ? '' : 'collapsed' }}"
                    aria-hidden="{{ $isDataAkademikOpen ? 'false' : 'true' }}">
            ...
            @if ($showMasterDosen || $showDosenPembimbing || $showDosenPenguji)
                ...
                <ul class="dropdown-menu {{ $isBimbinganOpen ? '' : 'collapsed' }}"
                    aria-hidden="{{ $isBimbinganOpen ? 'false' : 'true' }}">

                    @if ($showMasterDosen)
                        <li
                            class="{{ request()->is('dosen') || (request()->is('dosen/*') && !request()->is('dosen_penguji*')) ? 'active' : '' }}">
                    ...
                    @if ($showDosenPembimbing)
                        <li
                            class="{{ request()->is('datadosenpembimbing') || request()->is('datadosenpembimbing/*') ? 'active' : '' }}">
                    ...
                    @if ($showDosenPenguji)
                        <li
                            class="{{ request()->is('dosen_penguji') || request()->is('dosen_penguji/*') ? 'active' : '' }}">
            ...
            @if ($showDataPerusahaan)
                ...
                <ul class="dropdown-menu {{ $isPerusahaanOpen ? '' : 'collapsed' }}"
                    aria-hidden="{{ $isPerusahaanOpen ? 'false' : 'true' }}">
            ...
            @if ($showJadwal)
                ...
                <ul class="dropdown-menu {{ $isJadwalOpen ? '' : 'collapsed' }}"
                    aria-hidden="{{ $isJadwalOpen ? 'false' : 'true' }}">
            ...
            @if ($showNilaiPembimbing || $showNilaiPenguji || $showNilaiPerusahaan || $showNilaiMahasiswa || $showRatingPerusahaan)
                ...
                <ul class="dropdown-menu {{ $isPenilaianOpen ? '' : 'collapsed' }}"
                    aria-hidden="{{ $isPenilaianOpen ? 'false' : 'true' }}">

                    @if ($showNilaiPembimbing)
                        <li
                            class="{{ request()->is('penilaian') || (request()->is('penilaian/*') && !request()->is('penilaian_perusahaan*')) ? 'active' : '' }}">
                    ...
                    @if ($showNilaiPenguji)
                        <li
                            class="{{ request()->is('penilaian-penguji') || request()->is('penilaian-penguji/*') ? 'active' : '' }}">
                    ...
                    @if ($showNilaiPerusahaan)
                        <li
                            class="{{ request()->is('penilaian_perusahaan') || request()->is('penilaian_perusahaan/*') ? 'active' : '' }}">
                    ...
                    @if ($showRatingPerusahaan)
                        <li
                            class="{{ request()->is('ratingperusahaan') || request()->is('ratingperusahaan/*') ? 'active' : '' }}">
            ...
            {{-- MENU AHP & SAW --}}
            @if ($showAhpSaw)
                <h4 class="menu-dropdown-toggle" tabindex="0" data-persist-id="menu-ahp-saw">
                    <span>
                        <i class="fa fa-project-diagram"></i>
                        <span class="label-text">AHP & SAW</span>
                    </span>
                    <i class="fa fa-chevron-down dropdown-caret"></i>
                </h4>

                <ul class="dropdown-menu {{ $isAhpSawOpen ? '' : 'collapsed' }}"
                    aria-hidden="{{ $isAhpSawOpen ? 'false' : 'true' }}">

                    {{-- AHP --}}
                    <li class="{{ request()->is('ahp') || request()->is('ahp/*') ? 'active' : '' }}">
                        <a href="{{ route('ahp.index') }}">
                            <span class="label-text">Analytical Hierarchy Process</span>
                        </a>
                    </li>

                    {{-- SAW --}}
                    <li class="{{ request()->is('saw') || request()->is('saw/*') ? 'active' : '' }}">
                        <a href="{{ route('saw.index') }}">
                            <span class="label-text">Simple Additive Weighting</span>
                        </a>
                    </li>
                </ul>
            @endif
            ...
            <ul class="dropdown-menu {{ $isAkunOpen ? '' : 'collapsed' }}"
                aria-hidden="{{ $isAkunOpen ? 'false' : 'true' }}">
                <li>
                    <a href="#"
